...
$serviceList = Service::find()
    ->select('service_name')
    ->where(['in', 'id', $serviceListSubQuery])
    ->indexBy('id')
    ->orderBy('id')
    ->column();
    ...
    <aside class="filter-column trainer-filter-column">
        ...
        <div class="filter-column-top">
            ...
            <?php if ($trainers) {
                ...
                echo ($smallThumbsList == '') ? null : '<ul class="small-thumbs-list trainers-thumbs-list">' . $smallThumbsList . '</ul>';

            } ?>
            <span class="number trainers-number"><?= $trainersNumber . ' teacher' . (($trainersNumber !== 1) ? 's' : null) ?></span>
        </div>
        ...
        <ul class="trainers-list">
            ...
            <?php foreach ($trainers as $trainer) {
               ...
               echo "<li class='item trainer-item'>";
                ...
                echo "<h2 class='name item-name trainer-name'>" . Html::a($trainer->name . ' ' . $trainer->surname, ['/trainer/profile', 'id' => $trainer->id]) . '</h2>';
